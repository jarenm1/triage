...
def main():
    ...
    if args.trace is None:
        args.trace = Path(f"cuda/build/{args.task}-rollout.json")
    scope_name = f"{args.task}_rollout"
    ...
    environment = HoverEnv if args.task == "hover" else TrackingEnv
    with environment(args.num_envs) as env:
        ...
        with (
            ...
            torch.profiler.record_function(scope_name),
        ):
            learner.rollouts()
    ...
    scope = next(
        e for e in events if e.get("name") == scope_name and e.get("ph") == "X"
    )
